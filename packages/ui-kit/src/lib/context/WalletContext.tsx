...
export type WalletContextType = {
  ...
  handleExternalWalletSignRequest: (data: Uint8Array) => Promise<Uint8Array>;
...
export const WalletContextProvider: FC<PushWalletProviderProps> = ({
  ...
  const handleAppConnectionSuccess = (response: WalletEventRespoonse) => {
    ...
    if (response.account) {
      const result = PushChain.utils.account.toUniversal(response.account);
  ...
  const handleExternalWalletConnection = async (data: {
    ...
    try {
      ...
      const result = PushChain.utils.account.toUniversal(
