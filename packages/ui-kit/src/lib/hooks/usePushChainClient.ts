...
export const usePushChainClient = () => {
    ...
    const { config, universalAddress, handleSignMessage } = usePushWalletContext();
    ...
    useEffect(() => {
        const initializePushChain = async () => {
            ...
            try {
                ...
                const pushChainNetwork = config.env === ENV.LOCAL || config.env === ENV.TESTNET ? CONSTANTS.ENV.DEVNET : config.env;
