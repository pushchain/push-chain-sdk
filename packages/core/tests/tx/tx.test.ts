...
describe('Tx', () => {
  ...
  it('should serialize a Tx data into a Uint8Array', () => {
    const serializedTxData = PushChain.utils.tx.serializeData(
      mockInitDidTxData,
      TxCategory.INIT_DID
    );
  ...
  it('should deserialize a Uint8Array into a Tx Data object', () => {
    ...
    const deserializedTxData = PushChain.utils.tx.deserializeData(
      serializedTxData,
      TxCategory.INIT_DID
    );
  ...
  it('should verify a tx sign by evm address', async () => {
    ...
    const deserializedTx = PushChain.utils.tx.deserialize(
      hexToBytes(`0x${sentTx}`)
    );
