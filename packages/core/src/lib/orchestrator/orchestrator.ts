...
import type { OrchestratorContext } from './internals';
import {
  printLog as _printLog, fireProgressHook as _fireProgressHook,
  isPushChain as _isPushChain, getPushChainForNetwork as _getPushChainForNetwork,
  getChainNamespace as _getChainNamespace, chainFromNamespace as _chainFromNamespace,
  getNativePRC20ForChain as _getNativePRC20ForChain, getUniversalGatewayPCAddress as _getUniversalGatewayPCAddress,
  validateMainnetConnection as _validateMainnetConnection, bigintReplacer as _bigintReplacer,
  toExecuteParams as _toExecuteParams, buildUniversalTxRequest as _buildUniversalTxRequest,
  buildMulticallPayloadData as _buildMulticallPayloadData, getOriginGatewayContext as _getOriginGatewayContext,
  fetchOriginChainTransactionForProgress as _fetchOriginChainTransactionForProgress,
  signUniversalPayload as _signUniversalPayload, signMigrationPayload as _signMigrationPayload,
  computeExecutionHash as _computeExecutionHash, computeMigrationHash as _computeMigrationHash,
  encodeUniversalPayload as _encodeUniversalPayload, encodeUniversalPayloadSvm as _encodeUniversalPayloadSvm,
  waitForEvmConfirmationsWithCountdown as _waitForEvmConfirmationsWithCountdown,
  waitForSvmConfirmationsWithCountdown as _waitForSvmConfirmationsWithCountdown,
  waitForLockerFeeConfirmation as _waitForLockerFeeConfirmation,
  computeUniversalTxId as _computeUniversalTxId,
  ...
  reconstructProgressEvents as _reconstructProgressEvents,
  detectRouteFromUniversalTxData as _detectRouteFromUniversalTxData,
  transformToUniversalTxReceipt as _transformToUniversalTxReceipt,
  ensureErc20Allowance as _ensureErc20Allowance,
  queryOutboundGasFee as _queryOutboundGasFee, queryRescueGasFee as _queryRescueGasFee,
  calculateNativeAmountForDeposit as _calculateNativeAmountForDeposit,
  calculateGasAmountFromAmountOutMinETH as _calculateGasAmountFromAmountOutMinETH,
  computeUEAOffchain as _computeUEAOffchain, computeUEA as _computeUEA,
  getUEANonce as _getUEANonce, getUeaNonceForExecution as _getUeaNonceForExecution,
  getUeaStatusAndNonce as _getUeaStatusAndNonce, fetchUEAVersion as _fetchUEAVersion,
} from './internals';
import { buildSvmUniversalTxRequest as _buildSvmUniversalTxRequest, buildSvmUniversalTxRequestFromReq as _buildSvmUniversalTxRequestFromReq, getSvmProtocolFee as _getSvmProtocolFee, getSvmGatewayLogIndexFromTx as _getSvmGatewayLogIndexFromTx } from './internals/svm-helpers';
import { OUTBOUND_INITIAL_WAIT_MS, OUTBOUND_POLL_INTERVAL_MS, OUTBOUND_MAX_TIMEOUT_MS, waitForOutboundTx as _waitForOutboundTx, waitForAllOutboundTxsV2 as _waitForAllOutboundTxsV2 } from './internals/outbound-sync';
import { quoteExactOutput as _quoteExactOutput } from './internals/quote';
import { getAccountStatus as _getAccountStatus, upgradeAccount as _upgradeAccount, fetchLatestUEAVersion as _fetchLatestUEAVersion, migrateCEA as _migrateCEA } from './internals/account-manager';
import { queryUniversalTxStatusFromGatewayTx as _queryUniversalTxStatusFromGatewayTx, trackTransaction as _trackTransaction, transformToUniversalTxResponse as _transformToUniversalTxResponse, type ResponseBuilderCallbacks } from './internals/response-builder';
import { getGatewayVersion as _getGatewayVersion, isV1Gateway as _isV1Gateway, getGatewayAbiForVersion as _getGatewayAbiForVersion, getGatewayAbi as _getGatewayAbi, toGatewayRequestV1 as _toGatewayRequestV1, toGatewayRequest as _toGatewayRequest, toGatewayTokenRequestV1 as _toGatewayTokenRequestV1, toGatewayTokenRequest as _toGatewayTokenRequest, sendGatewayTxWithFallback as _sendGatewayTxWithFallback, sendGatewayTokenTxWithFallback as _sendGatewayTokenTxWithFallback, lockFee as _lockFee } from './internals/gateway-client';
import { executeMultiChain as _executeMultiChain, executeUoaToCea as _executeUoaToCea, executeUoaToCeaSvm as _executeUoaToCeaSvm, executeCeaToPush as _executeCeaToPush, executeCeaToPushSvm as _executeCeaToPushSvm, executeCeaToCea as _executeCeaToCea, buildPayloadForRoute as _buildPayloadForRoute } from './internals/route-handlers';
...
export class Orchestrator {
  ...
  private _getResponseCallbacks(): ResponseBuilderCallbacks {
    return {
      ...
      waitForOutboundTx: this.waitForOutboundTx.bind(this),
      transformToUniversalTxReceipt: this.transformToUniversalTxReceipt.bind(this),
      printLog: (msg: string) => this.printLog(msg),
      outboundConstants: { initialWaitMs: Orchestrator.OUTBOUND_INITIAL_WAIT_MS, pollingIntervalMs: Orchestrator.OUTBOUND_POLL_INTERVAL_MS, maxTimeoutMs: Orchestrator.OUTBOUND_MAX_TIMEOUT_MS },
    };
  }
  ...
  private async fetchLatestUEAVersion(vm: VM): Promise<string> {
    return _fetchLatestUEAVersion(this.ctx, vm);
  }
  ...
  private async executeUoaToCea(
    ...
    return _executeUoaToCea(this.ctx, params, this.execute.bind(this));
  }
  ...
  private async executeUoaToCeaSvm(
    ...
    return _executeUoaToCeaSvm(this.ctx, params, target, this.execute.bind(this));
  }
  ...
  private async executeCeaToPush(
    ...
    return _executeCeaToPush(this.ctx, params, this.execute.bind(this));
  }
  ...
  private async executeCeaToPushSvm(
    ...
    return _executeCeaToPushSvm(this.ctx, params, sourceChain, this.execute.bind(this));
  }
  ...
  private async buildPayloadForRoute(
    ...
    return _buildPayloadForRoute(this.ctx, params, route, nonce);
  }

  /**
   * Convert UniversalExecuteParams to ExecuteParams for backwards compatibility
   */
  private toExecuteParams(params: UniversalExecuteParams): ExecuteParams { return _toExecuteParams(params); }

  /**
   * Get the UniversalGatewayPC address for the current Push network
   */
  private getUniversalGatewayPCAddress(): `0x${string}` { return _getUniversalGatewayPCAddress(); }
   ...
   */
  private getNativePRC20ForChain(targetChain: CHAIN): `0x${string}` { return _getNativePRC20ForChain(targetChain, this.pushNetwork); }
  private getChainNamespace(chain: CHAIN): string { return _getChainNamespace(chain); }
  ...
  private async lockFee(
    ...
    return _lockFee(this.ctx, amount, universalPayload, req);
  }
  ...
  private async sendGatewayTxWithFallback(
    ...
    return _sendGatewayTxWithFallback(this.ctx, evmClient, address, req, signer, value);
  }
  ...
  private async sendGatewayTokenTxWithFallback(
    ...
    return _sendGatewayTokenTxWithFallback(this.ctx, evmClient, address, req, signer, value);
  }
  ...
  private async signUniversalPayload(
    ...
    return _signUniversalPayload(this.ctx, universalPayload, verifyingContract, version);
  }
  ...
  private computeExecutionHash({
    ...
    return _computeExecutionHash(this.ctx, { verifyingContract, payload, version });
  }
  ...
  private encodeUniversalPayloadSvm(payload: UniversalPayload): Buffer {
    return _encodeUniversalPayloadSvm(payload);
  }
  ...
  private reconstructProgressEvents(
    ...
    return _reconstructProgressEvents(universalTxResponse, universalTxData);
  }
   ...
   */
  private getPushChainForNetwork(): CHAIN { return _getPushChainForNetwork(this.pushNetwork); }
   ...
   */
  private computeUniversalTxId(pushChainTxHash: string): string { return _computeUniversalTxId(this.pushNetwork, pushChainTxHash); }
   ...
   */
  private chainFromNamespace(namespace: string): CHAIN | null { return _chainFromNamespace(namespace); }
  ...
  private async waitForOutboundTx(
    ...
    return _waitForOutboundTx(this.ctx, pushChainTxHash, options);
  }
  ...
  private async waitForAllOutboundTxsV2(
    ...
    return _waitForAllOutboundTxsV2(this.ctx, pushChainTxHash, outboundHops, options);
  }
  ...
  private async getUEANonce(address: `0x${string}`): Promise<bigint> {
    return _getUEANonce(this.ctx, address);
  }
  ...
  private async ensureErc20Allowance(
    ...
    return _ensureErc20Allowance(this.ctx, evmClient, tokenAddress, spender, requiredAmount);
  }

  /**
   * Ensures we're on Sepolia, returns EVM client and gateway address.
   */
  private getOriginGatewayContext(): { chain: CHAIN; evmClient?: EvmClient; gatewayAddress?: `0x${string}` } { return _getOriginGatewayContext(this.ctx); }

  /**
   * Computes UEA and fetches its nonce if deployed; returns 0 otherwise.
   */
  private async getUeaNonceForExecution(): Promise<bigint> { return _getUeaNonceForExecution(this.ctx); }
  private async getUeaStatusAndNonce(): Promise<{ deployed: boolean; nonce: bigint }> { return _getUeaStatusAndNonce(this.ctx); }
  ...
  private async transformToUniversalTxResponse(
    ...
    return _transformToUniversalTxResponse(this.ctx, tx, eventBuffer, this._getResponseCallbacks());
  }

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private bigintReplacer(_key: string, value: any) { return _bigintReplacer(_key, value); }
  private isPushChain(chain: CHAIN): boolean { return _isPushChain(chain); }
  private validateMainnetConnection(chain: CHAIN) { _validateMainnetConnection(chain, this.pushClient.pushChainInfo.chainId); }
  private printLog(log: string): void { _printLog(this.ctx, log); }
  private executeProgressHook(hookId: string, ...args: any[]): void { _fireProgressHook(this.ctx, hookId, ...args); }

  private getSvmGatewayLogIndexFromTx(txResp: any): number { return _getSvmGatewayLogIndexFromTx(txResp); }
  ...
  private async queryUniversalTxStatusFromGatewayTx(
    ...
    return _queryUniversalTxStatusFromGatewayTx(this.ctx, evmClient, gatewayAddress, txHash, evmGatewayMethod);
  }
  ...
  private async waitForEvmConfirmationsWithCountdown(
    ...
    return _waitForEvmConfirmationsWithCountdown(this.ctx, evmClient, txHash, confirmations, timeoutMs);
  }
  ...
  private async waitForSvmConfirmationsWithCountdown(
    ...
    return _waitForSvmConfirmationsWithCountdown(this.ctx, svmClient, txSignature, confirmations, timeoutMs);
  }

  private async fetchUEAVersion(): Promise<string> { return _fetchUEAVersion(this.ctx); }
