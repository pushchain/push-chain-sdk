...
describe('EOA → CEA: Outbound from Push Chain Native Account (Route 2)', () => {
  ...
  describe('Core Scenarios', () => {
    ...
    // ============================================================================
    // 8. Native Funds + Multicall — native funds + multicall outbound
    // ============================================================================
    describe('8. Native Funds + Multicall', () => {
      it('should transfer pBNB and increment counter from Push EOA', async () => {
        if (skipE2E) return;

        console.log('\n=== Test: EOA pBNB + Counter Increment (R2-PF-10) ===');
        ...
        // Multicall: send BNB + increment counter in one transaction
        const params: UniversalExecuteParams = {
          to: {
            address: TEST_TARGET,
            chain: CHAIN.BNB_TESTNET,
